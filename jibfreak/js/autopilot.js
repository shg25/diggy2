...
//   2. 警戒 — 脅威が中間距離(DANGER〜SAFE)なら、間合いを保って動かない
//   3. 位置取り — 脅威が SAFE より遠ければ定位置(HOME)へ戻る
//   4. 射撃 — 常に撃つ(連射間隔はゲーム側の仕様に従う)
//
...
export function autopilotActions(dt) {
	...
	// 目標地点を決める(ルール1: 回避 > ルール1.5: 警戒 > ルール2: 定位置)
	let target;
	if (threat && threat.dist > DANGER && threat.dist <= SAFE) {
		target = me; // 警戒: 中間距離の脅威とは間合いを保ち、その場で構える
	} else if (threat && threat.dist <= DANGER) {
