...
async function startInvincible(stage2 = false) {
	...
	await page.evaluate(() => {
		window.ridge.debug.state.muteki = true;
	});
}
...
test('1面: 撃破でスコアが入り、勝利でタイトルへ戻る', async () => {
	...
	await debug(() => {
		for (let i = 0; i < 20; i++) {
			ridge.debug.state.numTeki++;
			ridge.debug.makeTeki1();
		}
	});
	await debug(() => {
		for (let i = 0; i < 5; i++) ridge.debug.makePwr();
	});
...
test('2面: ボスが暴れモードで画面を周回し、敗北でタイトルへ戻る', async () => {
	...
	await debug(() => {
		ridge.debug.newSpriteBossSh2(1);
	});
	await page.waitForTimeout(1500);
	await debug(() => {
		ridge.debug.boss.set('life', 5);
	});
...
test('敵数カウンタは初回プレイから0以上の整数で推移する', async () => {
	await startInvincible();
	await debug(() => {
		for (let i = 0; i < 6; i++) ridge.debug.makePwr();
	});
...
test('3回プレイしてもスプライトプールとDOMが増えない(リーク検知)', async () => {
	...
	for (let game = 1; game <= 3; game++) {
		...
		await debug(() => {
			if (ridge.debug.isPlay()) ridge.debug.goLose();
		});
		await page.waitForTimeout(3800);
