...
test('回避を決めた直後はしばらく判断し直さない(往復の抑制・会長発議2)', () => {
	jiki.x = 284;
	jiki.y = 184; // 中心 (300, 200)
	tekis.push(tekiAt(350, 200)); // 右の近い脅威 → 左へ回避
	const dodge = autopilotActions(1);
	assert.ok(dodge.includes('left'));
	tekis.length = 0;
	tekis.push(tekiAt(250, 200)); // 今度は左に脅威が現れても…
	const held = autopilotActions(0.3); // 通常間隔(0.2秒)は過ぎている
	assert.deepEqual(held, dodge, '回避のやり切り中に判断し直した(往復が起きる)');
	const fresh = autopilotActions(0.2); // 回避のやり切り(0.45秒)を過ぎた
	assert.ok(fresh.includes('right'), '新しい脅威(左)から逃げていない');
});
