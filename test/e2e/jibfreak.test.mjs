...
/**
 * タイトルからゲーム開始し、戦闘が始まるまで待つ。
 * READY の2秒は「ゲーム内時間」なので壁時計の固定待ちにしない——
 * CI の遅い環境では dt 上限(loop.js の 0.1s 丸め)によりゲーム内時間が
 * 壁時計より遅く流れ、固定待ちや確率待ちが運負けする(大掃除#1の教訓)
 */
async function startGame() {
	await page.keyboard.press('Space');
	await page.waitForFunction(
		() => window.jibfreak.debug.stepFlg >= 11 && window.jibfreak.debug.stepFlg <= 13,
		null,
		{ timeout: 30000 },
	);
}
...
test('敵を撃つとスコアが入る', async () => {
	...
	await startGame();
	// 射線上に敵を置いて確実に当てる(敵が偶然通るのを待つ運任せは
	// CIの遅い環境で運負けする——大掃除#1の教訓)
	let score = 0;
...
test('敵に当たると GAME OVER になり、タイトルへ戻る', async () => {
	...
	// GAME OVER 3秒(ゲーム内時間)の後にタイトルへ
	await page.waitForFunction(() => window.jibfreak.debug.stepFlg === 0, null, { timeout: 20000 });
});

test('当たり判定: スプライトをかすめても死なず、中央の弱点に重なると死ぬ', async () => {
	await startGame();
	await page.waitForFunction(() => window.jibfreak.debug.tekis.length > 0, null, {
		timeout: 15000,
	});
...
test('ボム(B)で画面の敵がまとめて消えるが、スコアは入らない', async () => {
	await page.evaluate(() => {
		window.jibfreak.debug.state.muteki = true;
	});
	await startGame();
	...
	await page.waitForTimeout(300); // ボム窓(600ms)の途中、まだ湧きは再開していない
	// 実体のある敵(numTeki)で見る。総数(enemyCount)はやられ演出中の
	// 死骸も含むうえ、ボム窓が閉じた瞬間「敵が0なら必ず湧く」(classic)
	// が発動して新しい敵が現れるため、総数0を待つと不安定になる
	const numTekiAfter = await page.evaluate(() => window.jibfreak.debug.state.numTeki);
	assert.equal(numTekiAfter, 0, 'ボムで敵が消えない');
	// 大掃除#1後の追加(会長指摘): ボムでの一掃はスコアを入れない
	// (一気に大量加算されるとハイスコア狙いがボムありきになるため)
	const score = await page.evaluate(() => window.jibfreak.debug.score);
	assert.equal(score, 0, `ボムでスコアが入ってしまった: ${score}`);
});
...
test('アイテムが流れてくる(そして敵数を狂わせない)', async () => {
	...
	await startGame();
	// 平均6秒に1個湧く(毎tick 1/180)。壁時計でなくゲーム内時間で
	// 60秒(1800tick)まで待つ——確率は tick で刻まれるので、時間が
	// 希釈される環境でも判定の意味が変わらない
	let pwrs = 0;
	let ticks = 0;
	const deadline = Date.now() + 120000; // 壁時計の保険
	while (pwrs === 0 && ticks < 1800 && Date.now() < deadline) {
		await page.waitForTimeout(500);
		pwrs = await page.evaluate(() => window.jibfreak.debug.pwrCount);
		ticks = await page.evaluate(() => window.jibfreak.debug.state.counter);
	}
...
test('1面ボスを倒すと YOU WIN、スコアと装備を持ち越して2面へ続く(20年越しの初勝利)', async () => {
	...
	// 自機を動かしてから(持ち越しの検証用。初期位置(284,184)と同じ
	// ままだと「戻った」のか「保たれた」のか区別がつかない)、その位置に
	// 合わせてボスを瀕死・射線上へ引きずり出し、正規の当たり判定で倒す。
	// ボスの近くに置く(遠いと弾が届くまでの待ち時間が足りずフレークする)
	await page.evaluate(() => {
		window.jibfreak.debug.jiki.x = 350;
		window.jibfreak.debug.jiki.y = 250;
	});
	await page.evaluate(() => {
	...
	const posAtWin = await page.evaluate(() => ({ ...window.jibfreak.debug.jiki }));

	// 大掃除#1(元の設計の実現): タイトルへ戻らず、スコアを持ち越して2面へ。
	// YOU WIN の5秒はゲーム内時間なので、遷移そのものを待つ
	await page.waitForFunction(
		() => window.jibfreak.debug.stepFlg === 10 || window.jibfreak.debug.stepFlg === 11,
		null,
		{ timeout: 30000 },
	);
	const s = await step();
	...
	// 会長追加指摘: 地続き感を出すため、自機の位置も持ち越す
	const posAfter = await page.evaluate(() => ({ ...window.jibfreak.debug.jiki }));
	assert.equal(posAfter.x, posAtWin.x, '自機のx位置が初期化された');
	assert.equal(posAfter.y, posAtWin.y, '自機のy位置が初期化された');
});
...
test('ステージ2: 猫バスが「ニャッ」と鳴く(音のイベントとして記録される)', async () => {
	...
	await page.waitForFunction((b) => window.jibfreak.debug.soundRequests >= b + 2, base, {
		timeout: 45000,
	});
});
...
test('アトラクト: 放置でデモが始まり、CPUが撃ち、操作で復帰し、記録は残らない', async () => {
	// 何も操作せず放置する(タイトル15秒=ゲーム内時間でデモ開始)
